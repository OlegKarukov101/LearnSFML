...
public:
    ...
    void AddSpeedy(float speedy)   { 
        speed.y += speedy;  
        if (abs(speed.y) > 10) speed.y -= speedy;
    }
    ...
    void UpdateHero() {
        ...
        if (!isGround) speed.y += 0.2;
            ...
            if (speed.x > 0) { speed.x -= 0.3; if (speed.x < 0)speed.x = 0; }
            if (speed.x < 0) { speed.x += 0.3; if (speed.x > 0)speed.x = 0; }
        }
        Vector2f pos(0, 0);
...
int main()
{
    ...
    if (texture1.loadFromFile("texture2.jpeg",IntRect(0,20,128,128))) cout << "YEEES texture \n";
    //if (texture1.loadFromFile("texture1.jpeg",IntRect(10,10,128,128))) cout << "YEEES texture \n";
    ...
    for (int i = 0; i < infmapa.size(); i++) {
        for (int j = 0; j < infmapa[i].size(); j++) {
            ...
            if (infmapa[i][j] == '1') {
                if (tep1.second == pair<int, int>(0, 0))
                    tep1.second = pair<int, int>(i, j);
                else tep1.first = pair<int, int>(i, j);
