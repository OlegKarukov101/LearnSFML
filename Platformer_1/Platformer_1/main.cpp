...
int sz = 100;//dont touch
Physics ph;//constants for physics
...
public:
    ...
    void AddSpeedx(float speedx)   { 
        speed.x += speedx;  
        if (abs(speed.x) > ph.maxSpeedX) 
            speed.x -= speedx;
    }
    void AddSpeedy(float speedy)   { 
        speed.y += speedy;  
        if (abs(speed.y) > ph.maxSpeedY) speed.y -= speedy;
    }
    ...
    void UpdateHero() {
        ...
        if (!isGround) {
            ...
            if (speed.x != 0) {
                if (speed.x > 0) 
                    speed.x -= ph.groundFriction;
                else if (speed.x < 0) 
                    speed.x += ph.groundFriction;

                if (abs(speed.x) < ph.groundFriction) speed.x = 0;
            }
        }
        Vector2f pos(0, 0);
...
int main()
{
    ...
    RectangleShape fon = InitialRectangleShape(Vector2f(1400, 700),Color(130,255,255,0),Vector2f(800,450),sz,Color(50,220,50));
    Hero hero;
